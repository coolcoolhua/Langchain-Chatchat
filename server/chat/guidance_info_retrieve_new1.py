...
def history_preprocess(history):
    ...
    return extract_ret, True
        

def extract_info_sep(info,final_data,query):
    wrong_flag = False
...
def score_validation(scores,query):
    ...
    if len(scores) == 2 :
        if scores[1] != "未提及":
            ...
            if '~' in temp_score or '到' in temp_score or '-' in temp_score or '至' in temp_score:
                if '~' in temp_score:
                ...
                if '-' in temp_score:
                    ...
                    high_score = temp_score.split('-')[1]
                if '至' in temp_score:
                    low_score = temp_score.split('至')[0]
                    high_score = temp_score.split('至')[1]
...
def reformat_history_query(extract_infos,origin_data,query) -> dict:
    return_queries = {}
    for target_intent in extract_infos:
        if target_intent == 'career_locate':
            temp = origin_data['province']
            if temp != None:
                return_queries["career_locate"] = "我在" + temp['name']
        if target_intent == 'career_year':
            temp = origin_data['year']
            if temp != None:
                return_queries["career_year"] = "我想查" + str(temp) + "年的数据"
        if target_intent == 'career_major':
            temp = origin_data['intentionSpecialtyList']
            if len(temp) > 0:
                return_queries["career_major"] =  "我想学" + ','.join([t["name"] for t in temp])
        if target_intent == 'career_school_score':
            temp = origin_data['intentionScoreline']
            if len(temp) > 0:
                return_queries['career_school_score'] = "我在" + id2branch[temp['branch']] + "组考了" + str((temp['minScore'] + temp['maxScore'])/2) + "分"
        if target_intent == 'career_school_province':
            temp = origin_data['intentionProvinces']
            if len(temp) > 0:
                return_queries['career_school_province'] = "我想查" + ','.join([t['name'] for t in temp]) + "的大学"
        if target_intent == 'career_school_tag':
            temp = origin_data['tags']
            if len(temp) > 0:
                return_queries["career_school_tag"]  = "我想去" + ','.join(temp) + "类型的大学"
        if target_intent == 'career_schools':
            temp = origin_data['intentionUniversityList']
            if len(temp) > 0:
                return_queries["career_schools"]= "我想去" + ','.join([t['name'] for t in temp])
        if target_intent == 'career_selection':
            temp = origin_data['intentionSubjects']
            if len(temp) > 0:
                final_select = ""
                for t in temp:
                    final_select += ''.join([id2subject[z] for z in t])+','
                return_queries["career_selection"] = "我选了"+final_select
...
async def guidance_info_retrieve(
  ...
  stream: bool = Body(False, description="流式输出")
    ...
    async def process_items(prompt_name, semaphore,reformat_query=""):
        async with semaphore:
            # 在一个单独的线程中运行同步生成器，并获取所有内容
            if reformat_query =="":
                print("最终使用的query是",query)
                result = await asyncio.to_thread(lambda: list(api.chat_career(query, history=history, prompt_name=prompt_name)))
            else:
                print("最终使用的query是",reformat_query)
                result = await asyncio.to_thread(lambda: list(api.chat_career(reformat_query, history=history, prompt_name=prompt_name)))
            return result
    ...
    def agent_info_reformat(agent_info):
        res = []
        print("agent_info_type",type(agent_info))
    ...
    if not '相关' in judge_text and agent_info_reformat(agent_info) == []:
        ret['status'] = 401
    ...
    reformatted_queries = {}
    final_reformatted_queries = {}
    if agent_info != [] and (len(history) > 0 and extracted_check):
        extract_infos = agent_info_reformat(agent_info)
        print("经过agent后,最终意图",extract_infos)
        # 对query进行reformat
        reformatted_queries = reformat_history_query(extract_infos,origin_data,query)
        print("已经提取出的信息是",reformatted_queries)
        pre_extract_infos = ["career_continue_update"]
        if reformatted_queries != {}:
            for key in reformatted_queries:
                old_query = reformatted_queries[key]
                provide_query = "<学生历史信息>" + old_query + "</学生历史信息>" + "\n" + "<学生新提供的信息>" + query + "</学生新提供的信息>"
                async def pre_process_all_items():
                    semaphore = asyncio.Semaphore(1)  # 限制并发数为2
                    tasks = [process_items(extract_info, semaphore,provide_query) for extract_info in pre_extract_infos]
                    results = await asyncio.gather(*tasks)
                    for result, pre_extract_info in zip(results, pre_extract_infos):
                        res = ""
                        for item in result:
                            try:
                                ...
                                ret["data"] = json.dumps(origin_data)
                                return JSONResponse(ret)
                        print("agent1 res=",res)
                        try:
                            reformatted_queries[key] = str(res).replace("「","").replace("」","").replace('\n','')
                        except Exception as e:
                            print("agent_info1 error is", e)
                            reformatted_queries[key] = "query"
                    return reformatted_queries[key]
                final_reformatted_queries[key] = await pre_process_all_items()
        print("重新reformat后的query为",final_reformatted_queries)
    ...
    async def process_all_items():
        semaphore = asyncio.Semaphore(5)  # 限制并发数为2
        if final_reformatted_queries=={}:
            tasks = [process_items(extract_info, semaphore) for extract_info in extract_infos]
        else:
            print("最终query",final_reformatted_queries)
            tasks = [process_items(extract_info, semaphore, final_reformatted_queries.get(extract_info,query)) for extract_info in extract_infos]
        results = await asyncio.gather(*tasks)
    ...
    for info in all_info:
        origin_data,wrong_flag = extract_info_sep(info,origin_data,query)
