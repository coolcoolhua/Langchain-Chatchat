...
def get_recommend_strategy(major_nums,every_major_article_num,remainder):
    strategy = []
    for i in range(major_nums):
        if i < remainder:
            strategy.append(every_major_article_num + 1)
        else:
            strategy.append(every_major_article_num)
    return strategy
...
def get_recommend_articles(
    ...
    # 共有5个模块 1.专业类资讯 4.院校历史 5.院校招生政策
    ...
    # 构建查询
    all_university_history_articles = []
    for university in collegeTags["意向学校"]:
        print("意向学校", university)
        university_history_articles = {"title": university + "院校历史", "children": []}
        query = {
            "size": RETURNED_SCHOOL_HISTORY_NUM,  # 指定返回的文档数量
            "query": {
                "bool": {
                    "should": [
                        {
        ...
        if len(res["hits"]["hits"]) > 0:
            for article in res["hits"]["hits"]:
                ...
                usedUUIDs.append(article["_source"]["contentUUID"])
            all_university_history_articles.append(university_history_articles)

    # 5.院校招生政策, 查找所有relatedUniversitie和意向学校相同, 且topicTags为院校招生政策的文章
    all_university_policy_articles = []
    ...
    for university in collegeTags["意向学校"]:
        ...
        if len(res["hits"]["hits"]) > 0:
            for article in res["hits"]["hits"]:
                ...
                usedUUIDs.append(article["_source"]["contentUUID"])
            all_university_policy_articles.append(university_policy_articles)
    ...
    for i in range(len(majorTags["意向专业"])):
        ...
        if major_name in all_major_articles:
            print(len(all_major_articles[major_name][:articles_num_list[i]]))
            ret["interest_explore"][i]["children"].extend(all_major_articles[major_name][:articles_num_list[i]])
    
    # 按照策略分配文章         
    # for t in all_university_history_articles:
    #     ret["interest_explore"].append(t)
